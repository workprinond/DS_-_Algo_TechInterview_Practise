'''
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Question: Most Common Words
Amazon is partnering with the linguistics department at a local university to analyze important works of English literature and identify patterns in word usage across different eras.
To ensure a cleaner output, the linguistics department has provided a list of commonly used words (e.g. "an", "the". etc.) to exclude from the analysis.
...
The words "Jack", "Jill", "s", "to" and "cheese" have the next maximum frequency(two) in the given text but the words "Jack", "to" and "Jill" should be excluded
as these are commonly used words which you are not interested to include.
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
'''
import re, string
...
# re.sub(r'a', 'b', 'banana')
def mostCommonWords(text, exclude):
    #text = regex.sub('', text)
